...
// nodes,已经被读入数据的nodes对象。 lat1，building的坐标。 lon1，是building的坐标。
//
void Footways::findMinDist(Nodes &nodes ,double lat1 ,double lon1 , Footway &fw , long long & nid ,double & minDist)
{
  vector<long long> footwayIDs; 
  ...
  // 对于footways中的每一条路footway的每一个节点node，
  // 看看哪一个node离building最近
  // 找到最近的node后，返回它对应的footway路（后续需要输出该路的ID），node ID，最短距离
  for (Footway& F : this->MapFootways) // vector MapFootways是footways class里的，它存储着每条footway
  { 

        for (long long nodeid : F.NodeIDs) // 每条footway中有vector NodeIDs，存着这这条路里面的每个node ID，类型是long long
        { 
          double lat2 = 0.0;
          double lon2 = 0.0;
          bool entrance = false;

          // find()通过parameter返回lat2，lon2，entrance
          bool found = nodes.find(nodeid, lat2, lon2, entrance); 
